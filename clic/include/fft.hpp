...
clfftPlanHandle
bake_forward(const Array::Pointer & data)
{
  auto ocl_device = std::dynamic_pointer_cast<OpenCLDevice>(data->device());
  ...
  switch (data->dimension())
  {
    ...
    case 2:
      dim = CLFFT_2D;
      inStride[0] = 1;
      inStride[1] = (size_t)data->width();
      inStride[2] = 1;
      outStride[0] = 1;
      outStride[1] = (size_t)data->width() / 2 + 1;
      outStride[2] = 1;
      break;
    case 3:
      dim = CLFFT_3D;
      inStride[0] = 1;
      inStride[1] = (size_t)data->width();
      inStride[2] = (size_t)data->width() * data->height();
      outStride[0] = 1;
      outStride[1] = (size_t)data->width() / 2 + 1;
      outStride[2] = ((size_t)data->width() / 2 + 1) * data->height();
      break;
    default:
...
clfftPlanHandle
bake_backward(const Array::Pointer & data)
{
  auto ocl_device = std::dynamic_pointer_cast<OpenCLDevice>(data->device());
  auto ctx = ocl_device->getCLContext();
  auto queue = ocl_device->getCLCommandQueue();
  ...
  switch (data->dimension())
  {
    ...
    case 2:
      dim = CLFFT_2D;
      inStride[0] = 1;
      inStride[1] = (size_t)data->width() / 2 + 1;
      inStride[2] = 1;
      outStride[0] = 1;
      outStride[1] = (size_t)data->width();
      outStride[2] = 1;
      break;
    case 3:
      dim = CLFFT_3D;
      inStride[0] = 1;
      inStride[1] = (size_t)data->width() / 2 + 1;
      inStride[2] = ((size_t)data->width() / 2 + 1) * data->height();
      outStride[0] = 1;
      outStride[1] = (size_t)data->width();
      outStride[2] = (size_t)data->width() * data->height();
      break;
    default:
      throw std::runtime_error("Invalid FFT dimension");
  }
...
Array::Pointer
fft_forward(const Array::Pointer & input, Array::Pointer output)
{
...
Array::Pointer
fft_backward(const Array::Pointer & input, Array::Pointer output)
{
