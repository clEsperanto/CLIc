...
auto AllocateBufferObject (const ProcessorPointer &device,
                           const ShapeArray &shape, const BitType &type = CL_FLOAT) -> Image;

auto
AllocateImageObject (const ProcessorPointer &device,
                     const ShapeArray &shape, const BitType &type = CL_FLOAT) -> Image;
...
template <class type>
auto
WriteBufferObject (const Image &object,
                   const std::vector<type> &array) -> void
{
    size_t byte_length = array.size () * sizeof (type);
    Backend::EnqueueWriteToBuffer (object.Device ()->Queue (), object.Get (),
                                   true, 0, byte_length, array.data ());
}

template <class type>
auto
WriteImageObject (const Image &object,
                  const std::vector<type> &array) -> void
{
    Backend::EnqueueWriteToImage (object.Device ()->Queue (), object.Get (),
                                  CL_TRUE, object.Origin (), object.Shape (),
                                  array.data ());
}

template <class type>
auto
ReadBufferObject (const Image &object,
                  const std::vector<type> &array) -> void
{
    ...
    Backend::EnqueueReadFromBuffer (object.Device ()->Queue (), object.Get (),
                                    true, 0, byte_length,
                                    (void *)(array.data ()));
}
...
template <class type>
auto
ReadImageObject (const Image &object,
                 const std::vector<type> &array) -> void
{
    if (sizeof (type) != object.BitType ().Bytes ())
        {
    ...
    Backend::EnqueueReadFromImage (object.Device ()->Queue (), object.Get (),
                                   true, object.Origin (), object.Shape (),
                                   (void *)(array.data ()));
}
