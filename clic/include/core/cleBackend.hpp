#ifndef CLIC_INCLUDE_CORE_CLEBACKEND_HPP
#define CLIC_INCLUDE_CORE_CLEBACKEND_HPP
...
inline auto
GetDevicesListFromPlatform (const cl::Platform &platform_pointer, const DeviceType &device_type) -> std::vector<cl::Device>
{
    cl_int err = CL_SUCCESS;
...
inline auto
GetQueuePointer (const cl::Device &device_pointer, const cl::Context &context_pointer, const bool &in_order) -> cl::CommandQueue
{
...
inline auto
GetBufferPointer (const cl::Context &context_pointer, const MemAllocType &memory_alloc_mode, const HostAccessType &host_access_type, const KernelAccessType &kernel_access_type, const size_t size_in_bytes) -> cl::Memory
{
    cl_int err = CL_SUCCESS;
...
inline auto
GetImagePointer (const cl::Context &context_pointer, const MemAllocType &memory_alloc_mode, const HostAccessType &host_access_type, const KernelAccessType &kernel_access_type, const ChannelsType &image_channel_order, const DataType &image_channel_datatype, const std::array<size_t, 3> &shape) -> cl::Memory
{
    cl_int err = CL_SUCCESS;
    cl_mem_flags flags = memory_alloc_mode.Get () | host_access_type.Get () | kernel_access_type.Get ();
...
inline auto
BuildProgram (const cl::Program &program_pointer, const std::vector<cl::Device> &device_pointer, const std::string &options = "") -> bool
{
...
inline auto
BuildProgram (const cl::Program &program_pointer, const cl::Device &device_pointer, const std::string &options = "") -> bool
{
    cl_int err = program_pointer.build (device_pointer, options.c_str ());
    if (err != CL_SUCCESS)
        {
...
inline auto
EnqueueKernelExecution (const cl::CommandQueue &queue_pointer, const cl::Kernel &kernel_pointer, const std::array<size_t, 3> &global_size) -> void
{
...
inline auto
EnqueueReadFromBuffer (const cl::CommandQueue &queue_pointer, const cl::Memory &buffer_pointer, const bool &block_flag, const size_t &offset, const size_t &length_in_bytes, void *host_pointer) -> void
{
    const cl::Buffer memory (buffer_pointer.get (), true);
    cl_int err = queue_pointer.enqueueReadBuffer (memory, static_cast<cl_bool> (block_flag), offset, length_in_bytes, host_pointer, nullptr, nullptr);
    if (err != CL_SUCCESS)
        {
...
inline auto
EnqueueWriteToBuffer (const cl::CommandQueue &queue_pointer, const cl::Memory &buffer_pointer, const bool &block_flag, const size_t &offset, const size_t &length_in_bytes, const void *host_memory_pointer) -> void
{
    const cl::Buffer memory (buffer_pointer.get (), true);
    cl_int err = queue_pointer.enqueueWriteBuffer (memory, static_cast<cl_bool> (block_flag), offset, length_in_bytes, host_memory_pointer, nullptr, nullptr);
    if (err != CL_SUCCESS)
        {
...
inline auto
EnqueueReadFromBufferRegion (const cl::CommandQueue &queue_pointer, const cl::Memory &buffer_pointer, const bool &block_flag, const std::array<size_t, 3> &buffer_origin, const std::array<size_t, 3> &host_origin, const std::array<size_t, 3> &region, void *host_memory_pointer) -> void
{
    // cl_int err = CL_SUCCESS;
    // err = clEnqueueReadBufferRect(queue_pointer.get(), buffer_pointer.get(), block_flag, buffer_origin, host_origin, region, 0, 0, 0, 0, host_memory_pointer, 0, nullptr, nullptr);
    // if(err != CL_SUCCESS) { std::cerr << "Backend error in : " << GetOpenCLErrorInfo(err) << std::endl;}
}

inline auto
EnqueueWriteToBufferRegion (const cl::CommandQueue &queue_pointer, const cl::Memory &buffer_pointer, const bool &block_flag, const std::array<size_t, 3> &buffer_origin, const std::array<size_t, 3> &host_origin, const std::array<size_t, 3> &region, void *host_memory_pointer) -> void
{
    // cl_int err = CL_SUCCESS;
    // err = clEnqueueWriteBufferRect(queue_pointer.get(), buffer_pointer.get(), block_flag, buffer_origin, host_origin, region, 0, 0, 0, 0, host_memory_pointer, 0, nullptr, nullptr);
    // if(err != CL_SUCCESS) { std::cerr << "Backend error in : " << GetOpenCLErrorInfo(err) << std::endl;}
}

template <class NativeType>
inline auto
EnqueueFillBuffer (const cl::CommandQueue &queue_pointer, const cl::Memory &buffer_pointer, const bool &block_flag, const size_t &offset, const size_t &length_in_bytes, const NativeType &pattern) -> void
{
    const cl::Buffer memory (buffer_pointer.get (), true);
...
inline auto
EnqueueCopyBuffer (const cl::CommandQueue &queue_pointer, const cl::Memory &src_buffer_pointer, const cl::Memory &dst_buffer_pointer, const bool &block_flag, const size_t &src_offset, size_t dst_offset, const size_t &length_in_bytes) -> void
{
    const cl::Buffer src_memory (src_buffer_pointer.get (), true);
    const cl::Buffer dst_memory (dst_buffer_pointer.get (), true);
    cl_int err = queue_pointer.enqueueCopyBuffer (src_memory, dst_memory, src_offset, dst_offset, length_in_bytes, nullptr, nullptr);
    if (err != CL_SUCCESS)
        {
...
inline auto
EnqueueCopyBufferRegion (const cl::CommandQueue &queue_pointer, const cl::Memory &src_buffer_pointer, const cl::Memory &dst_buffer_pointer, const bool &block_flag, const std::array<size_t, 3> &src_origin, const std::array<size_t, 3> &dst_origin, const std::array<size_t, 3> &region) -> void
{
    // cl_int err = CL_SUCCESS;
    // err = clEnqueueCopyBufferRect(queue_pointer.get(), src_buffer_pointer.get(), dst_buffer_pointer.get(), src_origin, dst_origin, region, 0, 0, 0, 0, 0, nullptr, nullptr);
    // if(err != CL_SUCCESS) { std::cerr << "Backend error in : " << GetOpenCLErrorInfo(err) << std::endl;}
    // if(block_flag) { WaitQueueToFinish(queue_pointer); }
}

inline auto
EnqueueCopyBufferToImage (const cl::CommandQueue &queue_pointer, const cl::Memory &src_buffer_pointer, const cl::Memory &dst_image_pointer, const bool &block_flag, const size_t &src_offset, const std::array<size_t, 3> &dst_origin, const std::array<size_t, 3> &dst_region) -> void
{
    cl_int err = CL_SUCCESS;
    const cl::Buffer src_memory (src_buffer_pointer.get (), true);
        ...
        case CL_MEM_OBJECT_IMAGE1D:
            {
                ...
                err = queue_pointer.enqueueCopyBufferToImage (src_memory, dst_memory, src_offset, dst_origin, dst_region, nullptr, nullptr);
                break;
            }
...
inline auto
EnqueueCopyImageToBuffer (const cl::CommandQueue &queue_pointer, const cl::Memory &src_image_pointer, const cl::Memory &dst_buffer_pointer, const bool &block_flag, const std::array<size_t, 3> &src_origin, const std::array<size_t, 3> &src_region, size_t dst_offset) -> void
{
    cl_int err = CL_SUCCESS;
    const cl::Buffer dst_memory (dst_buffer_pointer.get (), true);
        ...
        case CL_MEM_OBJECT_IMAGE1D:
            {
                ...
                err = queue_pointer.enqueueCopyImageToBuffer (src_memory, dst_memory, src_origin, src_region, dst_offset, nullptr, nullptr);
                break;
            }
        case CL_MEM_OBJECT_IMAGE2D:
            {
...
inline auto
EnqueueReadFromImage (const cl::CommandQueue &queue_pointer, const cl::Memory &image_pointer, const bool &block_flag, const std::array<size_t, 3> &origin, const std::array<size_t, 3> &region, void *host_memory_pointer) -> void
{
    cl_int err = CL_SUCCESS;
        ...
        case CL_MEM_OBJECT_IMAGE1D:
            {
                ...
                err = queue_pointer.enqueueReadImage (memory, static_cast<cl_bool> (block_flag), origin, region, 0, 0, host_memory_pointer, nullptr, nullptr);
                break;
            }
        case CL_MEM_OBJECT_IMAGE2D:
            {
...
inline auto
EnqueueWriteToImage (const cl::CommandQueue &queue_pointer, const cl::Memory &image_pointer, const bool &block_flag, const std::array<size_t, 3> &origin, const std::array<size_t, 3> &region, const void *host_memory_pointer) -> void
{
    cl_int err = CL_SUCCESS;
    switch (image_pointer.getInfo<CL_MEM_TYPE> ())
        {
        case CL_MEM_OBJECT_IMAGE1D:
            {
                const cl::Image1D memory (image_pointer.get (), true);
                err = queue_pointer.enqueueWriteImage (memory, static_cast<cl_bool> (block_flag), origin, region, 0, 0, host_memory_pointer, nullptr, nullptr);
                break;
            }
        case CL_MEM_OBJECT_IMAGE2D:
            {
...
template <class NativeType>
inline auto
EnqueueFillImage (const cl::CommandQueue &queue_pointer, const cl::Memory &image_pointer, const bool &block_flag, const std::array<size_t, 3> &origin, const std::array<size_t, 3> &region, const NativeType &pattern) -> void
{
    cl_int err = CL_SUCCESS;
    switch (image_pointer.getInfo<CL_MEM_TYPE> ())
        {
...
inline auto
EnqueueCopyImage (const cl::CommandQueue &queue_pointer, const cl::Memory &src_image_pointer, const cl::Memory &dst_image_pointer, const bool &block_flag, const std::array<size_t, 3> &src_origin, const std::array<size_t, 3> &dst_origin, const std::array<size_t, 3> &region) -> void
{
    cl_int err = CL_SUCCESS;
    switch (src_image_pointer.getInfo<CL_MEM_TYPE> ())
        {
...
#endif // CLIC_INCLUDE_CORE_CLEBACKEND_HPP
