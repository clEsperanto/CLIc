...
MaskedVoronoiLabelingKernel::Execute() -> void
{
  ...
  auto flup = Memory::AllocateMemory(this->GetDevice(), src->Shape(), FLOAT32, src->GetMemoryType());
  auto flip = Memory::AllocateMemory(this->GetDevice(), src->Shape(), FLOAT32, src->GetMemoryType());
  auto flop = Memory::AllocateMemory(this->GetDevice(), src->Shape(), FLOAT32, src->GetMemoryType());
