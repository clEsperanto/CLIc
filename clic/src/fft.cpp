...
auto bake_forward(const Array::Pointer & real) -> clfftPlanHandle
{
  ...
  switch (real->dimension())  
  {
      ...
      case 3:
          ...
          outStride = {1, static_cast<size_t>(real->width()) / 2 + 1, (static_cast<size_t>(real->width()) / 2 + 1) * real->height()};
          break;
      default:
...
auto bake_backward(const Array::Pointer & real) -> clfftPlanHandle
{
  ...
  switch (real->dimension())  
  {
      ...
      case 3:
          dim = CLFFT_3D;
          inStride = {1, static_cast<size_t>(real->width()) / 2 + 1, (static_cast<size_t>(real->width()) / 2 + 1) * real->height()};
...
create_hermitian(const Array::Pointer & real_buf)
{
  ...
  return Array::create(
    hermitian_width * 2, real_buf->height(), real_buf->depth(), real_buf->dimension(), real_buf->dtype(), real_buf->mtype(), real_buf->device());
...
auto fft_forward(const Array::Pointer & real, Array::Pointer complex) -> void
{
  ...
  err = clfftEnqueueTransform(
    planHandle, CLFFT_FORWARD, 1, &queue, 0, NULL, NULL, static_cast<cl_mem *>(*real->get()), static_cast<cl_mem *>(*complex->get()), NULL);
...
auto fft_backward(const Array::Pointer & complex, Array::Pointer real) -> void
{
  ...
  /* Execute the plan. */
  err = clfftEnqueueTransform(
    planHandle, CLFFT_BACKWARD, 1, &queue, 0, NULL, NULL, static_cast<cl_mem *>(*complex->get()), static_cast<cl_mem *>(*real->get()), NULL);
...
auto conv(const Array::Pointer & input, const Array::Pointer & psf, Array::Pointer output, bool correlate) -> Array::Pointer
{
  ...
  // FFT of PSF
  clfftEnqueueTransform(forward_plan, CLFFT_FORWARD, 1, &queue, 0, NULL, NULL, static_cast<cl_mem *>(*psf->get()), static_cast<cl_mem *>(*fft_psf->get()), NULL);
  // FFT of image
  clfftEnqueueTransform(forward_plan, CLFFT_FORWARD, 1, &queue, 0, NULL, NULL, static_cast<cl_mem *>(*input->get()), static_cast<cl_mem *>(*fft_input->get()), NULL);
  
  // Multiply in frequency domain
  constexpr size_t LOCAL_ITEM_SIZE = 64;
  size_t globalItemSize = static_cast<size_t>(ceil(static_cast<double>(nElts) / static_cast<double>(LOCAL_ITEM_SIZE)) * LOCAL_ITEM_SIZE);
  size_t globalItemSizeFreq = static_cast<size_t>(ceil(static_cast<double>(nFreq) / static_cast<double>(LOCAL_ITEM_SIZE)) * LOCAL_ITEM_SIZE);
  std::string kernel_name = correlate ? "vecComplexConjugateMultiply" : "vecComplexMultiply";
  execOperationKernel(kernel_name, fft_input, fft_psf, fft_input, nFreq, { globalItemSizeFreq, 1, 1 }, { LOCAL_ITEM_SIZE, 1, 1 });

  // Inverse to get convolved
  clfftEnqueueTransform(backward_plan, CLFFT_BACKWARD, 1, &queue, 0, NULL, NULL, static_cast<cl_mem *>(*fft_input->get()), static_cast<cl_mem *>(*output->get()), NULL);
...
auto execTotalVariationTerm(const Array::Pointer & estimate, const Array::Pointer & correction, const Array::Pointer & variation, size_t nx, size_t ny, size_t nz, float hx, float hy, float hz, float regularization_factor, const RangeArray& global_range, const RangeArray& local_range) -> void
{
  ...
  const ParameterList params = { 
    { "estimate", estimate }, { "correction", correction }, { "variation", variation }, 
    { "Nx", nx }, {"Ny", ny},   {"Nz", nz}, 
    { "hx", hx }, { "hy", hy }, {"hz", hz}, 
...
auto deconv(const Array::Pointer & observe, const Array::Pointer & psf, Array::Pointer normal, Array::Pointer estimate, size_t iterations, float regularization) -> Array::Pointer
{
  ...
  constexpr size_t LOCAL_ITEM_SIZE = 64;
  size_t globalItemSize = static_cast<size_t>(ceil(static_cast<double>(nElts) / static_cast<double>(LOCAL_ITEM_SIZE)) * LOCAL_ITEM_SIZE);
  size_t globalItemSizeFreq = static_cast<size_t>(ceil(static_cast<double>(nFreq+1000) / static_cast<double>(LOCAL_ITEM_SIZE)) * LOCAL_ITEM_SIZE);
...
{
    // FFT of estimate
    clfftEnqueueTransform(forward_plan, CLFFT_FORWARD, 1, &queue, 0, NULL, NULL, static_cast<cl_mem *>(*estimate->get()), static_cast<cl_mem *>(*fft_estimate->get()), NULL);

    // complex multiply estimate FFT and PSF FFT
    execOperationKernel("vecComplexMultiply", fft_estimate, fft_psf, fft_estimate, nFreq, { globalItemSizeFreq, 1, 1 }, { LOCAL_ITEM_SIZE, 1, 1 });

    // Inverse to get reblurred
    clfftEnqueueTransform(backward_plan, CLFFT_BACKWARD, 1, &queue, 0, NULL, NULL, static_cast<cl_mem *>(*fft_estimate->get()), static_cast<cl_mem *>(*reblurred->get()), NULL);

    // divide observed by reblurred
    execOperationKernel("vecDiv", observe, reblurred, reblurred, nElts, { globalItemSize, 1, 1 }, { LOCAL_ITEM_SIZE, 1, 1 });

    // FFT of observed/reblurred
    clfftEnqueueTransform(forward_plan, CLFFT_FORWARD, 1, &queue, 0, NULL, NULL, static_cast<cl_mem *>(*reblurred->get()), static_cast<cl_mem *>(*fft_estimate->get()), NULL);

    // Correlate above result with PSF 
    execOperationKernel("vecComplexConjugateMultiply", fft_estimate, fft_psf, fft_estimate, nFreq, { globalItemSizeFreq, 1, 1 }, { LOCAL_ITEM_SIZE, 1, 1 });
    ...
    if(use_tv)
    {
      ...
      execTotalVariationTerm(
        estimate, reblurred, variation, 
        observe->width(), observe->height(), observe->depth(), 
        1.0, 1.0, 3.0, regularization, 
        { globalItemSize, 1, 1 }, { LOCAL_ITEM_SIZE, 1, 1 }
      );
      // multiply by variation factor
      execOperationKernel("vecMul", estimate, variation, estimate, nElts, { globalItemSize, 1, 1 }, { LOCAL_ITEM_SIZE, 1, 1 });
    }
    else
    {
      // multiply estimate by update factor
      execOperationKernel("vecMul", estimate, reblurred, estimate, nElts, { globalItemSize, 1, 1 }, { LOCAL_ITEM_SIZE, 1, 1 });
    }

    if (normal != nullptr)
    {
      // divide estimate by normal, if used
      execOperationKernel("vecDiv", estimate, normal, estimate, nElts, { globalItemSize, 1, 1 }, { LOCAL_ITEM_SIZE, 1, 1 });
    }
