...
auto CUDADevice::getInfo() const -> std::string {
    std::ostringstream result;
      size_t totalGlobalMem;
      int driverVersion, sharedMemPerBlock,runtimeVersion, regsPerBlock, warpSize, maxThreadsPerBlock, totalConstMem, major, minor, clockRate, textureAlignment, multiProcessorCount, maxBlockDimX, maxBlockDimY, maxBlockDimZ, maxGridDimX, maxGridDimY, maxGridDimZ;
...
cuDeviceGetAttribute(&maxGridDimZ, CU_DEVICE_ATTRIBUTE_MAX_GRID_DIM_Z, cudaDevice);
      ...
      result << std::left << std::setw(30) << "\tVendor:" << "NVIDIA Corporation" << "\n";
      ...
      result << std::left << std::setw(30) << "\tDevice type:" << "GPU" << "\n";
...
auto CUDADevice::getInfoExtended() const -> std::string {
    return getInfo()
}
