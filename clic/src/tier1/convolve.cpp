...
auto
convolve_global_func(const Device::Pointer & device, const Array::Pointer & src0, const Array::Pointer & src1, Array::Pointer dst)
  -> Array::Pointer
{
                     ...
                     const int               max_radius) -> Array::Pointer
{
  ...
  const ConstantList  constants = {
    { "TILE_X", tile[0] }, { "TILE_Y", tile[1] }, { "TILE_Z", tile[2] }, { "MAX_KERNEL_RADIUS", max_radius }
  };

  // Round global range up to tile boundaries
  const RangeArray global_range = { round_up(dst->width(), tile[0]), round_up(dst->height(), tile[1]), round_up(dst->depth(), tile[2]) };
  ...
  -> Array::Pointer
{
  ...
  const size_t shared_mem_size = (tile[0] + 2 * max_radius) * (tile[1] + 2 * max_radius) * (tile[2] + 2 * max_radius) * sizeof(float);
