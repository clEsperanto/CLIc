...
{
  tier0::create_dst(coordinate_list1, distance_matrix_destination, coordinate_list1->width() + 1, coordinate_list1->width() + 1, 1, dType::FLOAT);
  ...
  const RangeArray    range = { distance_matrix_destination->width(), distance_matrix_destination->height(), distance_matrix_destination->depth() };
