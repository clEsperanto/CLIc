                ...
                Array::Pointer          dst) -> Array::Pointer
{
  ...
  const KernelInfo    kernel_code = { "binary_and", kernel::binary_and };
...
binary_not_func(const Device::Pointer & device, const Array::Pointer & src, Array::Pointer dst) -> Array::Pointer
{
  ...
  const KernelInfo    kernel_code = { "binary_not", kernel::binary_not };
...
{
  tier0::create_like(src0, dst, dType::BINARY);
  const KernelInfo    kernel_code = { "binary_or", kernel::binary_or };
  const ParameterList params = { { "src0", src0 }, { "src1", src1 }, { "dst", dst } };
  const RangeArray    range = { dst->width(), dst->height(), dst->depth() };
  execute(device, kernel_code, params, range);
  return dst;
...
{
  tier0::create_like(src0, dst, dType::BINARY);
  const KernelInfo    kernel_code = { "binary_subtract", kernel::binary_subtract };
  const ParameterList params = { { "src0", src0 }, { "src1", src1 }, { "dst", dst } };
  const RangeArray    range = { dst->width(), dst->height(), dst->depth() };
  execute(device, kernel_code, params, range);
  return dst;
...
{
  tier0::create_like(src0, dst, dType::BINARY);
  const KernelInfo    kernel_code = { "binary_xor", kernel::binary_xor };
  const ParameterList params = { { "src0", src0 }, { "src1", src1 }, { "dst", dst } };
  const RangeArray    range = { dst->width(), dst->height(), dst->depth() };
  execute(device, kernel_code, params, range);
  return dst;
...
{
  tier0::create_like(src, dst, dType::BINARY);
  const KernelInfo    kernel_code = { "binary_edge_detection", kernel::binary_edge_detection };
  const ParameterList params = { { "src", src }, { "dst", dst } };
  const RangeArray    range = { dst->width(), dst->height(), dst->depth() };
  execute(device, kernel_code, params, range);
  return dst;
