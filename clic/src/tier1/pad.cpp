         ...
         bool                    center) -> Array::Pointer
{
  ...
  const auto pad_x = std::abs(static_cast<float>(size_x - src->width()));
  const auto pad_y = std::abs(static_cast<float>(size_y - src->height()));
  const auto pad_z = std::abs(static_cast<float>(size_z - src->depth()));
           ...
           bool                    center) -> Array::Pointer
{
  ...
  const auto pad_x = std::abs(static_cast<float>(src->width() - size_x));
  const auto pad_y = std::abs(static_cast<float>(src->height() - size_y));
  const auto pad_z = std::abs(static_cast<float>(src->depth() - size_z));
