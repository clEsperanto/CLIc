...
// std::string copy = R"CLC(
// __kernel void KERNEL_NAME(__global const INDTYPE* restrict src,
//                     __global OUTDTYPE* restrict dst,
//                     const uint n)
// {
//     const uint gid = get_global_id(0);
//     if (gid < n)
//         dst[gid] = CONVERT(src[gid]);
// })CLC";
...
copy_func(const Device::Pointer & device, const Array::Pointer & src, Array::Pointer dst) -> Array::Pointer
{
  ...
  const ParameterList params = { { "src", src }, { "dst", dst }, { "n", static_cast<uint>(dst->size()) } };
