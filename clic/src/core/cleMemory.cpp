...
auto
AllocateBufferObject (const ProcessorPointer &device,
                      const ShapeArray &shape, const BitType &type) -> Image
{
    ...
    auto object = Backend::GetBufferPointer (
        device->Context (), mem_alloc, host_access,
        kernel_access, byte_length);

    ObjectType object_type = { Backend::GetMemoryType (object) };

    return Image (device, object, shape, data_type, object_type,
                  channels_type);
}
                     ...
                     const ShapeArray &shape, const BitType &type) -> Image
{
    ...
    auto object = Backend::GetImagePointer (
        device->Context (), mem_alloc, host_access,
        kernel_access, channels_type, data_type, shape);
...
auto
AllocateObject (const ProcessorPointer &device, const ShapeArray &shape, const BitType &type, const MemType &object) -> Image
{
