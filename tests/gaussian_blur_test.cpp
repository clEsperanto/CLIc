...
run_test (const std::array<size_t, 3> &shape, const cl_mem_object_type &mem_type) -> bool
{
    ...
    std::transform (output.begin (), output.end (), output.begin (), [] (const type &x) { return static_cast<type> (std::round (static_cast<float> (x) * 10) / 10); });
    std::transform (valid.begin (), valid.end (), valid.begin (), [] (const type &x) { return static_cast<type> (std::round (static_cast<float> (x) * 10) / 10); });
