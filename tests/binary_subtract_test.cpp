                                   ...
                                   std::vector<type>& valid, size_t width, size_t height, size_t depth)
{
    ...
    while(it_arr1 != arr_1.end() && it_arr2 != arr_2.end() && it_valid != valid.end())
    {
        ...
        *it_valid = ((*it_arr1) - (*it_arr2) == 0) ? 0 : (*it_arr1) ;
        ++it_arr1;
        ++it_arr2;
        ++it_valid;
    }
...
std::vector<type> run_kernel_with_buffer(std::vector<type>& arr_1, std::vector<type>& arr_2, std::array<size_t,3>& shape)
{
    ...
    cle.BinarySubtract(oclArray_A, oclArray_B, ocl_output);  
...
int main(int argc, char **argv)
{
    ...
    {
        std::cerr << "BinarySubtract kernel 3d ... FAILED! " << std::endl;
        return EXIT_FAILURE;
    }
    if (test<float>(10, 5,  1))
    {
        std::cerr << "BinarySubtract kernel 2d ... FAILED! " << std::endl;
        return EXIT_FAILURE;
    }
    if (test<float>(10,  1,  1))
    {        
        std::cerr << "BinarySubtract kernel 1d ... FAILED! " << std::endl;
        return EXIT_FAILURE;
    }
    
    std::cout << "BinarySubtract kernel test ... PASSED! " << std::endl;
    return EXIT_SUCCESS;
}