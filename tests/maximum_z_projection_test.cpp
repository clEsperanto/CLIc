...
run_kernel_with_buffer (std::vector<type> &arr_1, std::array<size_t, 3> &shape)
{
    ...
    cle.MaximumZProjection (oclArray_A, ocl_output);
...
main (int argc, char **argv)
{
    if (test<float> (5, 3, 2))
        {
            std::cerr << "MaximumZProjection kernel 3d ... FAILED! " << std::endl;
            return EXIT_FAILURE;
        }
    if (test<float> (5, 3, 1))
        {
            std::cerr << "MaximumZProjection kernel 2d ... FAILED! " << std::endl;
            return EXIT_FAILURE;
        }
    if (test<float> (5, 1, 1))
        {
            std::cerr << "MaximumZProjection kernel 1d ... FAILED! " << std::endl;
            return EXIT_FAILURE;
        }
    std::cout << "MaximumZProjection kernel test ... PASSED! " << std::endl;
    return EXIT_SUCCESS;
}
