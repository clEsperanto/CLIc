...
run_kernel_with_buffer (std::vector<type> &arr_1, std::array<size_t, 3> &shape)
{
    ...
    cle.OnlyzeroOverwriteMaximumDiamond (oclArray_A, ocl_flag, ocl_output);
...
main (int argc, char **argv)
{
    if (test<float> (5, 5, 1))
        {
            std::cerr << "OnlyzeroOverwriteMaximumDiamond kernel 2d ... FAILED! " << std::endl;
            return EXIT_FAILURE;
        }

    std::cout << "OnlyzeroOverwriteMaximumDiamond kernel test ... PASSED! " << std::endl;
    return EXIT_SUCCESS;
}