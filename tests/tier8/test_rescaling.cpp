...
TEST_P(TestMakeIsotropic, makeIsotropicWithExplicitTargetSpacing)
{
  ...
  auto gpu_output = cle::tier8::make_isotropic_func(device, gpu_input, nullptr, 2.0, 1.0, 1.0, 1.0, true);
...
TEST_P(TestMakeIsotropic, makeIsotropicWithDefaultTargetSpacing)
{
  ...
  auto gpu_output = cle::tier8::make_isotropic_func(device, gpu_input, nullptr, 2.0, 1.0, 0.5, -1.0, true);
...
TEST_P(TestMakeAnisotropic, makeAnisotropicWithExplicitTargetSpacing)
{
  ...
  auto gpu_output = cle::tier8::make_anisotropic_func(device, gpu_input, nullptr, 1.0, 2.0, 1.0, 1.0, true);
...
INSTANTIATE_TEST_SUITE_P(InstantiationName, TestMakeAnisotropic, ::testing::ValuesIn(getParameters()));
