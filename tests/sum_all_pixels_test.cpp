...
bool IsDifferent(std::vector<type>& output, std::vector<type>& valid)
{
    ...
    {
        std::cerr << "[FAILED] : output size does not match. " << output.size() << " != " << valid.size() << std::endl;
        return true;
    }
    float difference = 0;
...
std::vector<type> run_kernel_with_buffer(std::vector<type>& arr_1, std::array<size_t,3>& shape)
{
    ...
    cle.SumOfAllPixels(oclArray_A, ocl_output);  
...
std::vector<type> run_kernel_with_image(std::vector<type>& arr_1, std::array<size_t,3>& shape)
{
    ...
    auto ocl_output = cle.CreateImage<type>();
    cle.SumOfAllPixels(oclArray_A, ocl_output);  
...
int main(int argc, char **argv)
{
    ...
    {
        std::cerr << "SumOfAllPixels kernel 3d ... FAILED! " << std::endl;
        return EXIT_FAILURE;
    }
    if (test<float>(10, 5,  1))
    {
        std::cerr << "SumOfAllPixels kernel 2d ... FAILED! " << std::endl;
        return EXIT_FAILURE;
    }
    if (test<float>(10,  1,  1))
    {        
        std::cerr << "SumOfAllPixels kernel 1d ... FAILED! " << std::endl;
        return EXIT_FAILURE;
    }
    std::cout << "SumOfAllPixels kernel test ... PASSED! " << std::endl;
    return EXIT_SUCCESS;
}