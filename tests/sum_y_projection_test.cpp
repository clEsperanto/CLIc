                                   ...
                                   std::vector<type>& valid, size_t width, size_t height, size_t depth)
{
    ...
    std::fill(valid.begin(), valid.end(), height);
...
int main(int argc, char **argv)
{
    if (test<float>(5, 3, 2))
    {
        std::cerr << "SumYProjection kernel 3d ... FAILED! " << std::endl;
        return EXIT_FAILURE;
    }
    if (test<float>(5, 3, 1))
    {
        std::cerr << "SumYProjection kernel 2d ... FAILED! " << std::endl;
        return EXIT_FAILURE;
    }
    if (test<float>(5, 1, 1))
    {        
        std::cerr << "SumYProjection kernel 1d ... FAILED! " << std::endl;
        return EXIT_FAILURE;
    }
    std::cout << "SumYProjection kernel test ... PASSED! " << std::endl;
    return EXIT_SUCCESS;
}
