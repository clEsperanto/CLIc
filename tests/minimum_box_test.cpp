               ...
               std::vector<type> &valid, size_t width, size_t height, size_t depth)
{
    ...
    for (auto i = 0; i < arr_1.size (); ++i)
        {
            if ((i >= central_idx - width - 1 && i <= central_idx - width + 1) || (i >= central_idx - 1 && i <= central_idx + 1) || (i >= central_idx + width - 1 && i <= central_idx + width + 1) || (i >= central_idx - (height * width) - width - 1 && i <= central_idx - (height * width) - width + 1) || (i >= central_idx - (height * width) - 1 && i <= central_idx - (height * width) + 1) || (i >= central_idx - (height * width) + width - 1 && i <= central_idx - (height * width) + width + 1) || (i >= central_idx + (height * width) - width - 1 && i <= central_idx + (height * width) - width + 1) || (i >= central_idx + (height * width) - 1 && i <= central_idx + (height * width) + 1) || (i >= central_idx + (height * width) + width - 1 && i <= central_idx + (height * width) + width + 1))
                {
...
run_kernel_with_buffer (std::vector<type> &arr_1, std::array<size_t, 3> &shape)
{
    ...
    cle.MinimumBox (oclArray_A, ocl_output, 1, 1, 1);
...
main (int argc, char **argv)
{
    if (test<float> (10, 5, 3))
        {
            std::cerr << "MinimumBox kernel 3d ... FAILED! " << std::endl;
            return EXIT_FAILURE;
        }
    if (test<float> (10, 5, 1))
        {
            std::cerr << "MinimumBox kernel 2d ... FAILED! " << std::endl;
            return EXIT_FAILURE;
        }
    if (test<float> (10, 1, 1))
        {
            std::cerr << "MinimumBox kernel 1d ... FAILED! " << std::endl;
            return EXIT_FAILURE;
        }
    std::cout << "MinimumBox kernel test ... PASSED! " << std::endl;
    return EXIT_SUCCESS;
}
