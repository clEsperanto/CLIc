                        ...
                        std::vector<type> &arr_2, std::array<size_t, 3> &shape)
{
    ...
    cle.SmallerOrEqual (oclArray_A, oclArray_B, ocl_output);
...
main (int argc, char **argv)
{
    if (test<float> (10, 5, 2))
        {
            std::cerr << "SmallerOrEqual kernel 3d ... FAILED! " << std::endl;
            return EXIT_FAILURE;
        }
    if (test<float> (10, 5, 1))
        {
            std::cerr << "SmallerOrEqual kernel 2d ... FAILED! " << std::endl;
            return EXIT_FAILURE;
        }
    if (test<float> (10, 1, 1))
        {
            std::cerr << "SmallerOrEqual kernel 1d ... FAILED! " << std::endl;
            return EXIT_FAILURE;
        }
    std::cout << "SmallerOrEqual kernel test ... PASSED! " << std::endl;
    return EXIT_SUCCESS;
}