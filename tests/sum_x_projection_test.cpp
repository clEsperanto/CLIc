                                   ...
                                   std::vector<type>& valid, size_t width, size_t height, size_t depth)
{
    ...
    std::fill(valid.begin(), valid.end(), static_cast<type>(width));
...
int main(int argc, char **argv)
{
    if (test<float>(5, 3, 2))
    {
        std::cerr << "SumXProjection kernel 3d ... FAILED! " << std::endl;
        return EXIT_FAILURE;
    }
    if (test<float>(5, 3, 1))
    {
        std::cerr << "SumXProjection kernel 2d ... FAILED! " << std::endl;
        return EXIT_FAILURE;
    }
    if (test<float>(5, 1, 1))
    {        
        std::cerr << "SumXProjection kernel 1d ... FAILED! " << std::endl;
        return EXIT_FAILURE;
    }
    std::cout << "SumXProjection kernel test ... PASSED! " << std::endl;
    return EXIT_SUCCESS;
}
