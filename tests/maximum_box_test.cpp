...
run_kernel_with_buffer (std::vector<type> &arr_1, std::array<size_t, 3> &shape)
{
    ...
    cle.MaximumBox (oclArray_A, ocl_output, 1, 1, 1);
...
main (int argc, char **argv)
{
    if (test<float> (10, 5, 3))
        {
            std::cerr << "MaximumBox kernel 3d ... FAILED! " << std::endl;
            return EXIT_FAILURE;
        }
    if (test<float> (10, 5, 1))
        {
            std::cerr << "MaximumBox kernel 2d ... FAILED! " << std::endl;
            return EXIT_FAILURE;
        }
    if (test<float> (10, 1, 1))
        {
            std::cerr << "MaximumBox kernel 1d ... FAILED! " << std::endl;
            return EXIT_FAILURE;
        }
    std::cout << "MaximumBox kernel test ... PASSED! " << std::endl;
    return EXIT_SUCCESS;
}
