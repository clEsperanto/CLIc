...
template <class type>
std::array<size_t, 3>
generate_data (std::vector<type> &arr_1, std::vector<type> &valid, size_t width, size_t height, size_t depth, float scalar)
{
...
template <class type>
std::vector<type>
run_kernel_with_buffer (std::vector<type> &arr, std::array<size_t, 3> &shape, float scalar)
{
    cle::Clesperanto cle;
    cle.GetDevice ()->WaitForKernelToFinish ();
    auto oclArray_A = cle.Push<type> (arr, shape);
    auto ocl_output = cle.Create<type> (shape);
    cle.SmallerOrEqualConstant (oclArray_A, ocl_output, scalar);
...
template <class type>
std::vector<type>
run_kernel_with_image (std::vector<type> &arr, std::array<size_t, 3> &shape, float scalar)
{
    cle::Clesperanto cle;
    cle.GetDevice ()->WaitForKernelToFinish ();
...
main (int argc, char **argv)
{
    if (test<float> (10, 5, 2))
        {
            std::cerr << "SmallerOrEqualConstant kernel 3d ... FAILED! " << std::endl;
            return EXIT_FAILURE;
        }
    if (test<float> (10, 5, 1))
        {
            std::cerr << "SmallerOrEqualConstant kernel 2d ... FAILED! " << std::endl;
            return EXIT_FAILURE;
        }
    if (test<float> (10, 1, 1))
        {
            std::cerr << "SmallerOrEqualConstant kernel 1d ... FAILED! " << std::endl;
            return EXIT_FAILURE;
        }
    std::cout << "SmallerOrEqualConstant kernel test ... PASSED! " << std::endl;
    return EXIT_SUCCESS;
}