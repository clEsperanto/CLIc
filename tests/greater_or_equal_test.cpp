...
template <class type>
std::vector<type>
run_kernel_with_buffer (std::vector<type> &arr_1,
                        std::vector<type> &arr_2, std::array<size_t, 3> &shape)
{
    ...
    cle.GreaterOrEqual (oclArray_A, oclArray_B, ocl_output);
...
template <class type>
std::vector<type>
run_kernel_with_image (std::vector<type> &arr_1,
                       std::vector<type> &arr_2, std::array<size_t, 3> &shape)
{
    cle::Clesperanto cle;
    cle.GetDevice ()->WaitForKernelToFinish ();
...
main (int argc, char **argv)
{
    if (test<float> (10, 5, 2))
        {
            std::cerr << "GreaterOrEqual kernel 3d ... FAILED! " << std::endl;
            return EXIT_FAILURE;
        }
    if (test<float> (10, 5, 1))
        {
            std::cerr << "GreaterOrEqual kernel 2d ... FAILED! " << std::endl;
            return EXIT_FAILURE;
        }
    if (test<float> (10, 1, 1))
        {
            std::cerr << "GreaterOrEqual kernel 1d ... FAILED! " << std::endl;
            return EXIT_FAILURE;
        }
    std::cout << "GreaterOrEqual kernel test ... PASSED! " << std::endl;
    return EXIT_SUCCESS;
}