...
class TestModuloImages : public ::testing::TestWithParam<std::string>
{
protected:
  ...
  SetUp()
  {
    ...
    std::fill(valid.begin(), valid.end(), static_cast<float>(std::fmod(value1, value2)));
  }
};

TEST_P(TestModuloImages, execute)
{
...
INSTANTIATE_TEST_SUITE_P(InstantiationName, TestModuloImages, ::testing::ValuesIn(getParameters()));
