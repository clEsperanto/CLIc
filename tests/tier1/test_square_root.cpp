...
class TestSquareRoot : public ::testing::TestWithParam<std::string>
{
protected:
  const float                   value = 9;
  ...
  SetUp()
  {
    ...
    std::fill(valid.begin(), valid.end(), static_cast<float>(std::sqrt(value)));
  }
};

TEST_P(TestSquareRoot, execute)
{
...
INSTANTIATE_TEST_SUITE_P(InstantiationName, TestSquareRoot, ::testing::ValuesIn(getParameters()));
